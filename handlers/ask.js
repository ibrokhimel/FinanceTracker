...
import { getSpendingSummary, getTotalSpentThisMonth } from '../db/queries/expenses.js';
...
export async function handleAsk(bot, msg) {
  ...
  if (!question) {
    return bot.sendMessage(chatId,
      ...
      `\`/ask should I cut my dining budget?\`\n` +
      `\`/ask what's my biggest savings opportunity?\`\n` +
      `\`/ask compare last month to this month\``,
      { parse_mode: 'Markdown' });
  }
  ...
  try {
    ...
    const messages = [
      { role: 'system', content: `You are a personal finance assistant. Answer in 2-4 short, friendly sentences based on the user's actual data shown below. Never invent numbers; if data is missing, say so. The user's local currency is UZS unless otherwise specified.\n\n=== USER DATA ===\n${ctx}\n=== END DATA ===` },
      { role: 'user',   content: question },
    ];
    const res = await chat(messages, { temperature: 0.6, maxTokens: 400, userId, purpose: 'ask' });

    if (!res.ok) {
      return bot.sendMessage(chatId, `❌ The AI didn't reply (${res.error || 'no provider available'}). Try again or check /usage.`);
    }
    const tag = res.cached ? ' 💾' : '';
    await bot.sendMessage(chatId, `🤖${tag} ${res.text}\n\n_via ${res.provider}_`, { parse_mode: 'Markdown' });
  } catch (err) {
